...
const deleteUser = async () => {
  try {
    ...
    const response = await makeApiRequest('DELETE', `${BASE_URL}/api/employee/delete-user/${uid}`);
...
const grantFreeAccess = async () => {
  try {
    ...
    const response = await makeApiRequest('POST', `${BASE_URL}/api/employee/grant-free-access`, {
      userId,
      freeChannels,
      freeGroups,
    });
...
const getPlatformStats = async () => {
  try {
    const response = await makeApiRequest('GET', `${BASE_URL}/api/employee/platform-stats`);
...
const uploadConversationProfilePhoto = async () => {
  try {
    ...
    const response = await makeMultipartApiRequest('POST', `${BASE_URL}/api/employee/conversation/${conversationId}/upload-photo`, {
      filePath: imagePath,
...
const pinMessage = async () => {
  try {
    ...
    const response = await makeApiRequest('POST', `${BASE_URL}/api/employee/conversation/pin-message`, {
      conversationId,
      messageId,
    });
...
const renameConversation = async () => {
  try {
    ...
    const response = await makeApiRequest('POST', `${BASE_URL}/api/employee/conversation/rename`, {
      conversationId,
      newName,
    });
