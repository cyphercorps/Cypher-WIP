...
const updateProfile = async () => {
  try {
    ...
    const privacySettings = await promptForInput(
      "Enter privacy settings (JSON format):",
    );
...
const uploadProfilePhoto = async () => {
  try {
    const uid = await promptForInput("Enter your UID:");
    const filePath = await promptForInput(
      "Enter the path to your profile photo:",
    );
