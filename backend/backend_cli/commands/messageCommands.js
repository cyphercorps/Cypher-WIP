...
const sendMessage = async () => {
  try {
    ...
    const selfDestructTime = await promptForInput(
      "Enter self-destruct time in milliseconds (optional):",
      true,
    );
...
const editMessage = async () => {
  try {
    ...
    const response = await makeApiRequest(
      "POST",
      `${BASE_URL}/api/messages/edit-message`,
      {
        conversationId,
        messageId,
        newMessage,
...
const updateReadReceipts = async () => {
  try {
    ...
    const response = await makeApiRequest(
      "POST",
      `${BASE_URL}/api/messages/update-read-receipts`,
      {
        conversationId,
        userId,
      },
