...
const sendNotification = async () => {
  try {
    ...
    const response = await makeApiRequest('POST', `${BASE_URL}/api/notifications/send`, {
      recipientId,
      title,
      message,
    });
...
const getUserNotifications = async () => {
  try {
    ...
    const response = await makeApiRequest('GET', `${BASE_URL}/api/notifications/${userId}`);
...
const markNotificationAsRead = async () => {
  try {
    ...
    const response = await makeApiRequest('PUT', `${BASE_URL}/api/notifications/${userId}/${notificationId}/read`);
